 ...
 */
export async function sendEmailAction(_to: string, _onderwerp: string, _body: string): Promise<{ ok: boolean; melding: string }> {
  await requireAdmin();
  if (!process.env.RESEND_API_KEY) {
    return { ok: false, melding: "Verzenden is nog niet gekoppeld. Voeg RESEND_API_KEY toe om e-mails echt te versturen." };
  }
  // TODO Resend: daadwerkelijk versturen
