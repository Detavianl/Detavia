  ...
  return (
    <form action={createDeal} className="mx-auto max-w-2xl p-8">
      ...
      <div className="mt-8 grid gap-5">
        ...
        <label className="grid min-w-0 gap-1.5"><span className="text-sm font-bold">Fase</span>
          <select name="stage" className="w-full rounded-xl border-2 border-neutral-200 bg-white px-4 py-3">
...
function Field({ label, name, type = "text", required = false, placeholder }: { label: string; name: string; type?: string; required?: boolean; placeholder?: string }) {
  return (
    <label className="grid min-w-0 gap-1.5">
      ...
      <input name={name} type={type} required={required} placeholder={placeholder} className="w-full rounded-xl border-2 border-neutral-200 px-4 py-3" />
    </label>
  );
}
